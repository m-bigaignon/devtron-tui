...
| `devtron-api/tests/sse.rs` | START/END framing, `STAGE_INFO`, reconnect with `Last-Event-ID`, pod stream close (FR-014, US4 scenario 5, FR-028) |
...
| M4 | US1 | Point `token_file` to a file holding an invalid token | "token … was rejected", not a crash. `:instances` is still usable. |
...
| M13 | edge | `kill -9` the process, and separately press `Ctrl-C` | In both cases the shell is usable without running `reset`. |
